...
剪影係透明 SVG，分層（牆、頂、門窗、物件分開畫），字喺 DOM，圖入面冇字。圖書館、商店沿用 #25 A 嘅等角檔。農場、競技場、燈塔係加咗細節同慢動作嘅樣板（`catalog.html` 有舊版並排）；其餘五座未改。動作寫喺 SVG 入面嘅 CSS keyframes，開「減少動態」就停，只留靜態底圖。撳格算法冇改。

樣板動作（慢、輕）：競技場兩邊旗 2.8 秒同 3.4 秒輕輕搖，兩個小朋友對打 3.2 秒一循環（無武器）；農場風車葉 12 秒轉一圈，雞行同啄 6 秒；燈塔燈光 14 秒轉一圈，海鷗飛 8 秒、翼拍 1.1 秒。
